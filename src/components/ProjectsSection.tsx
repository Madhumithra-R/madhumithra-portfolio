...
const ProjectsSection = () => {
  const ref = useReveal();
  return (
    <section id="projects" className="relative z-[1] min-h-screen px-5 md:px-16 lg:px-20 py-20 md:py-24 bg-dark">
      ...
      <div ref={ref} className="reveal grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-[1100px] mx-auto">
        {projects.map((p) => (
          <div key={p.title} className="card-dark overflow-hidden">
            <div className="h-36 md:h-40 flex items-center justify-center text-4xl border-b border-border" style={{ background: "linear-gradient(135deg, hsl(195,50%,12%), hsl(200,40%,9%))" }}>
              {p.icon}
            </div>
            <div className="p-5 md:p-6">
              <h4 className="text-sm md:text-base font-bold mb-2 font-heading">{p.title}</h4>
              <p className="text-xs md:text-sm text-muted-foreground leading-[1.65] mb-4">{p.desc}</p>
